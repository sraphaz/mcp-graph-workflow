...
const log = createLogger({ layer: "core", source: "entity-store.ts" });
...
export class EntityStore {
  ...
  findByName(query: string, limit: number = 10): Entity[] {
    ...
    try {
      ...
      log.debug("intentional-swallow", { error: String(err), reason: "FTS query syntax error — fall through to LIKE" });
    }
