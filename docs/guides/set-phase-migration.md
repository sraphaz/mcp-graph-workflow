# Migration — MCP `set_phase` tool → `mcp-graph set-phase` CLI

The MCP tool `set_phase` (called via `mcp__mcp-graph__set_phase` from inside an agent session) is **deprecated in v11** and **removed in v12.0**. Replacement: the `mcp-graph set-phase` CLI command (Sprint 7.4 #7.4.7).
...
| Version | MCP `set_phase` status | `mcp-graph set-phase` status | What you'd get |
|---|---|---|---|
...
`mcp-graph set-phase` and the deprecated MCP tool **share the same SQLite store**. Latest-write wins; you can mix entry points within a single project without inconsistency.

| Aspect | MCP `set_phase` | `mcp-graph set-phase` |
|---|---|---|
...
| Visible to Claude Code agents | ✅ during the agent's own session | ✅ if the agent runs `mcp-graph set-phase` via Bash, or if SessionStart drift hook re-reads |
...
```text
Run: mcp-graph set-phase REVIEW
```
...
# new
mcp-graph set-phase IMPLEMENT
```

### In `.claude/settings.local.json` SessionStart

The `balanced` and `aggressive` hook profiles already call `mcp-graph hook session-start`, which detects when the recorded phase mismatches the graph state and prompts a `mcp-graph set-phase` rerun. No change needed if you ran `mcp-graph hooks install`.
...
You don't need this flag to migrate. It only affects whether the *hook* enforces gates pre-tool-call. The CLI command `mcp-graph set-phase` enforces gates regardless (driven by `--mode`).
...
- E1: [`mcp-graph hooks` CLI](./hooks.md)
- E2: [`mcp-graph set-phase`](./set-phase.md)
- E3: [Hook event reference](./hooks-event-reference.md)
