...
describe("finishTask", () => {
  ...
  it("should mark task as done when DoD passes", () => {
  ...
  it("should return blocked when DoD fails", () => {
    ...
    const result = finishTask(store, bareTask.id, { autoNext: false });

    expect(result.status).toBe("blocked");
    expect(result.blockers.length).toBeGreaterThan(0);
  });

  it("should index rationale as decision when done", () => {
    const result = finishTask(store, taskId, {
  ...
  it("should detect epic promotion when all siblings done", () => {
  ...
  it("should not return next task when autoNext is false", () => {
    const result = finishTask(store, taskId, { autoNext: false });

    expect(result.nextTask).toBeNull();
  });

  it("should update testFiles when provided", () => {
    const result = finishTask(store, taskId, {
