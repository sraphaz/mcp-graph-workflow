...
const log = createLogger({ layer: "core", source: "rag-pipeline.ts" });
...
export async function incrementalIndex(
  ...
  for (const docId of docIds) {
    ...
    try {
      ...
      log.debug("intentional-swallow", { error: String(err), reason: "graceful degradation for incremental embedding" });
    }
