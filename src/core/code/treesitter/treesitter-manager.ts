...
function resolveGrammarPath(languageId: string): string | null {
  ...
  try {
    ...
    log.debug("intentional-swallow", { error: String(err), reason: "bundled path not found — fall through to npm-package strategies" });
  }
  ...
  try {
    ...
    log.debug("intentional-swallow", { error: String(err), reason: "not found at CWD level — try relative to this package" });
  }
